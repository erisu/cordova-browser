...
module.exports.run = function (args) {
    ...
    const manifestFilePath = path.resolve(path.join(args.www, 'manifest.json'));

    // Default start page when manifest.json is missing
    let startPage = 'index.html';

    // Try to get the start page URL from manifest
    if (fs.existsSync(manifestFilePath)) {
        try {
            const manifest = require(manifestFilePath);
            // If start_url is empty, fallback to the original default.
            startPage = manifest.start_url || startPage;
        } catch (err) {
